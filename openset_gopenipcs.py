...
from parse_args import parse_args_openset_openipcs
...
def train_model_pca(args, net):
    ...
    val_loader = DataLoader(valid_dataset, batch_size=1, num_workers=args.num_workers, shuffle=False)
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(val_loader):
            ...
            if args.model == 'seg_former': # gopenipcs
                ...
                _, features = net(img, mask)
                decode_head = net.model.decode_head
                outs, last_classifier = decode_head(features)
                outs = interpolate(outs, size=(224,224), mode="bilinear", align_corners=False)
                last_classifier = interpolate(last_classifier, size=(224,224), mode="bilinear", align_corners=False)
                soft_outs = F.softmax(outs, dim=1)
                prds = soft_outs.data.max(1)[1]
...
def compute_threshold_from_pca(args, net, model_full):
    ...
    # validation data
    args.split = 'train'
    _, val_dataset = get_dataset_train(args, get_validation_augmentation, get_validation_augmentation)
    _, val_view = get_dataset_train(args, get_view, get_view)
    num_known_classes = val_dataset._get_num_known_classes()
    
    view_loader = DataLoader(val_view, batch_size=1, num_workers=args.num_workers, shuffle=False)
    test_loader = DataLoader(val_dataset, batch_size=1, num_workers=args.num_workers, shuffle=False)
...
if __name__ == '__main__':
    ...
    args, parser = parse_args_openset_openipcs()
